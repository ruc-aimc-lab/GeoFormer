...
class FinePreprocess(nn.Module):
    ...
    def forward(self, feat_f0, feat_f1, feat_c0, feat_c1, data: Dict[str, torch.Tensor]):
        ...
        if data['b_ids'].shape[0] == 0:
            feat0 = torch.empty(0, int(self.W**2), int(self.d_model_f), device=feat_f0.device)
            feat1 = torch.empty(0, int(self.W**2), int(self.d_model_f), device=feat_f0.device)
            return feat0, feat1

        # 1. unfold(crop) all local windows
        feat_f0_unfold = F.unfold(feat_f0, kernel_size=(W, W), stride=int(stride), padding=W // 2)
        ...
        feat_f1_unfold = F.unfold(feat_f1, kernel_size=(W, W), stride=int(stride), padding=W // 2)
            ...
            feat_cf_win = self.merge_feat(
                torch.cat(
                    [
                        ...
                        feat_c_win.unsqueeze(1).repeat(1, int(W**2), 1),  # [2n, ww, cf]
                    ],
                    -1,
                )
            )
