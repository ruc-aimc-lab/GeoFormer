...
class LoFTRLoss(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config  # loftr_config under the global namespace
...
class GeoLoss(nn.Module):
    ...
    def compute_fine_loss(self, conf, conf_gt):
        pos_mask, neg_mask = conf_gt == 1, conf_gt == 0
        c_pos_w, c_neg_w = self.c_pos_w, self.c_neg_w
        conf = torch.clamp(conf, 1e-6, 1 - 1e-6)
        loss_pos = (- torch.log(conf[pos_mask])).mean()
        loss_neg = (- torch.log(1 - conf[neg_mask])).mean()
        if torch.isnan(loss_neg):
            return loss_pos
        if torch.isnan(loss_pos):
            return loss_neg
        return c_pos_w * loss_pos + c_neg_w * loss_neg
    ...
    def forward(self, data):
        ...
        loss_d = self.compute_coarse_loss(
            data['dect_conf_matrix'],
            data['conf_matrix_gt'],
            weight=c_weight)

        loss = (loss_c + loss_d) * self.loss_config['coarse_weight']
        loss_scalars.update({"loss_c": loss_c.clone().detach().cpu(), "loss_d": loss_d.clone().detach().cpu()})

        loss_f = self.compute_fine_loss(
            data['fine_matrix'],
            data['conf_matrix_fine_gt'])
        loss += loss_f * self.loss_config['fine_weight']
        loss_scalars.update({"loss_f": loss_f.clone().detach().cpu()})
        # # 2. fine-level loss
        # loss_f = self.compute_fine_loss(data['expec_f'], data['expec_f_gt'])
