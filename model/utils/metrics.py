...
def compute_homography_errors(data):
    m_bids = data['m_bids']
    pts0 = data['mkpts0_f']
    pts1 = data['mkpts1_f']
    epi_errs = []
    ...
    for bs in range(H_0to1.size(0)):
        ...
        p0_warp = warp_points_batch(p0.unsqueeze(0), H.unsqueeze(0))
        epi_errs.append((torch.sqrt(((p0_warp - p1) ** 2).sum(-1))).squeeze())
    try:
        epi_errs = torch.cat(epi_errs, dim=0)
    except Exception:
        epi_errs = torch.empty(H_0to1.size(0), 1).to(pts0.device)
