...
from utils.homography import sample_homography, affine_image
...
def resize_aspect_ratio(image, resize_h, resize_w, scale=32):
    h, w = image.shape[0:2]
    max_size = max(h, w)
    ratio_h, ratio_w = h / max_size, w / max_size

    new_height, new_width = int((resize_h * ratio_h // scale) * scale), int((resize_w * ratio_w // scale) * scale)
