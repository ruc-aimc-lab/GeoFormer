...
def init_empty_sfm(args):
    from eval_tool.immatch.utils.colmap.data_parsing import (
...
def reconstruct_database_pairs(args):
    from eval_tool.third_party.hloc.hloc import triangulation
...
def localize_queries(args):
    from eval_tool.third_party.hloc.hloc import localize_sfm
    ...
    with h5py.File(match_file, 'a') as fmatch:
        ...
        for pair, key in tqdm(zip(pairs, pair_keys), total=len(pairs), smoothing=.1):
            ...
            try:
                t0 = time.time()
                try:
                    match_res = matcher(im1_path, im2_path, False)
                except:
                    match_res = matcher(im1_path, im2_path)
                match_times.append(time.time() - t0)
            except Exception as e:
                print(f'##Run on CPU {key}')
                t0 = time.time()
                try:
                    match_res = matcher(im1_path, im2_path, True)
                except:
                    match_res = matcher(im1_path, im2_path)
                match_times.append(time.time() - t0)
                # continue
